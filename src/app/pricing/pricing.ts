...
export class PricingPage implements OnInit {
  ...
  comparisonData: ComparisonData = {
    ...
    platform: [
      ...
      },
      {
        name: 'User Limit',
        tiers: [
          { value: '1 User' }, { value: 'Up to 5 Users' }, { value: 'Up to 10 Users' }, { value: 'Custom' }
        ]
      },
      {
        name: 'Brand Limit',
  ...
  handleScroll() {
    ...
    const pricingCards = document.querySelector('.grid.md\\:grid-cols-4');
    
    if (stickyHeader && pricingCards) {
      const scrollY = window.scrollY || window.pageYOffset;
      const pricingCardsRect = pricingCards.getBoundingClientRect();
      const pricingCardsTop = pricingCardsRect.top + scrollY;
      
      // Show sticky header when pricing cards start to scroll out of view
      if (scrollY > pricingCardsTop + 100) {
  ...
  getStickyButtonText(tier: PricingTier): string {
    if (tier.price === 0) return 'Request a Quote';
    return tier.name; // Show plan name in sticky header
  }
